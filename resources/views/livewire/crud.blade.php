                ...
                <tbody >
                    @foreach ($items as $item)
                        <tr >
                            ...
                            <td class="p-0">
                                @if ($field['type'] === "file")
                                @if(strpos($item[$field['name']], 'http') === 0)
                                    <img src="{{$item[$field['name']]}}" alt="{{$item[$field['name']] }}" class="img-thumbnail rounded-circle imagen" width="60">
                                @elseif (file_exists(public_path('storage/' . $item[$field['name']])))
                            ...
                            @endforeach
                            <td class="p-0">
                                <button wire:click="editItemData({{$item}})"  data-bs-toggle="modal" data-bs-target="#myModal" class="btn btn-success btn-sm">  
                                    <i class="fas fa-pencil-alt"></i>
                                </button> 
...
<div class="modal fade" id="myModal" tabindex="-1" aria-labelledby="myModal Label" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                @if(Auth::check()) 
                <form action="{{ $item !='vaciar' ? route('admin.' . $modelName . '.update', $item) : route('admin.' . $modelName . '.store') }}"  method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        @switch($field['type'])
                            @case('textarea')
                                <textarea rows="2" cols="50" class="form-control" placeholder="{{$field['name']}}" name="{{$field['name']}}">{{ $item !='vaciar' ? $item[$field['name']] : old($field['name'])}}</textarea>
                                @break
                            @case('file')
                                @if ($item !='vaciar')
                                <div class="text-center">    
                                    <img src="{{ asset('storage/' . $item[$field['name']]) }}" alt="img" class="img-thumbnail imagen mx-auto" width="100">
                                </div>
                                @endif
                                <input class="form-control" name="{{$field['name']}}" type="file" accept="image/*" value="{{ $item !='vaciar' ? $item[$field['name']] : old($field['name'])}}">
                                @break
                            @case('password')
                                <input class="form-control"type="password" >
                                @break
                            @case('email')
                                <input class="form-control" name="{{$field['name']}}" type="email" placeholder="{{$field['name']}}" value="{{ $item !='vaciar' ? $item[$field['name']] : old($field['name'])}}">
                                @break
                            @case('select')
                                <select class="form-control" name="{{$field['name']}}">
                                    <option value="opcion1">Opción 1</option>
                            ...
                            @case('number')
                                <div class="input-group">
                                    <div class="input-group-append">
                                        <span class="input-group-text">$</span>
                                        <input class="form-control" name="{{$field['name']}}" type="number" placeholder="0" min="0" value="{{ $item !='vaciar' ? $item[$field['name']] : old($field['name'])}}">
                                    </div>
                                </div>
                                @break
                            @default
                          
                                <input  class="form-control" name="{{$field['name']}}" type="text" placeholder="{{$field['name']}}" value="{{ $item !='vaciar' ? $item[$field['name']] : old($field['name'])}}" >
                        @endswitch
                        @error($field['name'])
                            <span class="text-danger">{{$message}}</span>
                        @enderror
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cerrar</button>
                        <button type="submit" class="btn btn-primary">{{ $item != 'vaciar' ? 'Actualizar' : 'Crear' }}</button>
                    </div>
                </form>
                @endif
            </div>
